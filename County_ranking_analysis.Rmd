...
names(region_col)<-c("Association of Bay Area\nHealth Officers (ABAHO)", "Greater Sacramento Region\nHealth Officers (GSRHO)", "Rural Association of Northern\nCalifornia Health\nOfficers (RANCHO)", "San Joaquin Valley Consortium\n(SJVC)", "Southern California\nHealth Officers (SCAL)")   
...
A<-actuals_state %>% filter(index> as.Date("2021-02-01"), index<as.Date("2022-02-01")) %>% ggplot() + geom_line(aes(x=index, y=hospitalized)) + theme_classic()+ xlab("Date")+ ylab("Hospital Census")+ 
...
D<- regions %>% left_join(county_fips, by="county") %>%
  ...
  ggplot(mapping = aes(long, lat, group = group, fill = region_line_break)) +
...
pdf(file = "figures/fig1.pdf",  
    width = 6.69, # The width of the plot in inches, 170 mm=6.69"
    height = 6) # The height of the plot in inches, 225 mm=8.85"

print(cowplot::plot_grid(col_one, D, labels=c("", "D"), ncol=2, rel_widths=c(1.8,1)))

dev.off()

# cowplot::ggsave2("figures/fig1.pdf", width=6.69, height=7)
```
...
```{r compare_R_vax}
#Visualize reff_curves for one county from each of the health officer regions
reff_curve_cnty %>% rename(County=county) %>% filter(index> as.Date("2021-02-01"), index< as.Date("2022-02-01"), County %in% c("Contra Costa", "Butte", "Fresno", "Del Norte", "Los Angeles")) %>% ggplot()+ geom_line(aes(x=index, y=estimate, col=County))+ geom_hline(yintercept=1, lty="dashed")+ xlab("Date")+ ylab("R-effective Estimate") + theme_classic()+
geom_rect(data = variant_dates, aes(xmin = start_date, xmax = end_date, fill = variant_period), ymin = -Inf, ymax = Inf, alpha = 0.3)+  scale_fill_manual(name="Analysis\nPeriod", labels=variant_labels[names(variant_cols) %in% unique(variant_dates$variant_period)], values=variant_cols[names(variant_cols) %in% unique(variant_dates$variant_period)])

#Visualize vaccination coverage curves for one county from each of the health officer regions
vx %>% rename(County=county) %>% filter(index> as.Date("2021-02-01"), index< as.Date("2022-02-01"), County %in% c("Contra Costa", "Butte", "Fresno", "Del Norte", "Los Angeles")) %>% ggplot()+ geom_line(aes(x=index, y=perc_COVIDvx*100, col=County)) + xlab("Date")+ ylab("Vaccination Coverage (%)") + theme_classic()+
geom_rect(data = variant_dates, aes(xmin = start_date, xmax = end_date, fill = variant_period), ymin = -Inf, ymax = Inf, alpha = 0.3)+  scale_fill_manual(name="Analysis\nPeriod", labels=variant_labels[names(variant_cols) %in% unique(variant_dates$variant_period)], values=variant_cols[names(variant_cols) %in% unique(variant_dates$variant_period)])
```
## 7 days
...
MAE_rankings_omicron<-omicron_ranks %>% left_join(regions, by= "county") %>%  group_by(forecaster) %>% drop_na() %>%
  ...
  scale_fill_manual(name = "Model:", labels=names(forecaster_map_col)[names(forecaster_map_col) %in% unique(omicron_ranks$forecaster)], values=forecaster_map_col[names(forecaster_map_col) %in% unique(omicron_ranks$forecaster)] ) + theme_classic()+ xlab("Fractional Ranking")+ ylab("Frequency")+ 
  guides(fill="none")+ 
...
abbr <- c("perc_COVIDvx", "r_eff", "diff", "delta_frac", "pop_size", "other_frac", "alpha_frac", "gamma_frac", "omicron_frac", "income", "perc_unemploy", "education", "perc_pov")
var_imp_names<-data.frame(abbr, proper_names)

var_imp<-data.frame(abbr=row.names(fit1_varImp$importance), importance= fit1_varImp$importance$Overall) %>% left_join(var_imp_names) %>% arrange(desc(importance))

# fit1_varImp$importance[order(fit1_varImp$importance, decreasing=TRUE),]
...
MAE_heatmap_alpha<-alpha_daily_winner %>% left_join(regions, by= "county") %>% ggplot(aes(y=as.factor(county), x=as.Date(index),  fill=model))+
  geom_tile(color="white", size=0.001) + facet_grid(region_acronym~. , scale= "free")+
  scale_fill_manual(name = "Model:", labels=gglabels_MAE[names(MAE_map_col) %in% unique(alpha_daily_winner$model)], values=MAE_map_col[names(MAE_map_col) %in% unique(alpha_daily_winner$model)] )+
  scale_x_date(date_breaks = "7 days", date_labels = "%b %d", name="Publication Date")+
  ylab("County")+
  labs(fill = "Model:") +
  ...
  theme(axis.text.y = element_text(size=8), legend.position = "top")#+  ggtitle("Best daily performing model by county as measured by MAE")
...
col2<- cowplot::plot_grid(MAE_map_alpha, MAE_rankings_alpha, labels=c("B", "C"), ncol=1, align="hv", axis = "l")
cowplot::plot_grid(MAE_heatmap_alpha, col2, ncol=2, labels=c("A", ""), axis = "l", rel_widths = c(2.4,1))
...
col2<- cowplot::plot_grid(MAE_map_delta, MAE_rankings_delta, labels=c("B", "C"), ncol=1, align="hv", axis = "l")
cowplot::plot_grid(MAE_heatmap_delta, col2, ncol=2, labels=c("A", ""), axis = "l", rel_widths = c(2.4,1))
...
MAE_rankings_omicron<-omicron_ranks %>% left_join(regions, by= "county") %>%  group_by(forecaster) %>% drop_na() %>%
  ...
  geom_vline(aes(xintercept = med, group = forecaster), colour = 'black', lty="dashed")+ 
  geom_vline(aes(xintercept = avg, group = forecaster), colour = 'black', lty="solid")+ 
  facet_grid(forecaster ~., scale="fixed")+
  scale_fill_manual(name = "Model:", labels=names(forecaster_map_col)[names(forecaster_map_col) %in% unique(omicron_ranks$forecaster)], values=forecaster_map_col[names(forecaster_map_col) %in% unique(omicron_ranks$forecaster)] ) + theme_classic()+ xlab("Fractional Ranking")+ ylab("Frequency")+ 
  guides(fill="none")+ 
  theme(strip.text = element_blank())+
...
col2<- cowplot::plot_grid(MAE_map_omicron, MAE_rankings_omicron, labels=c("B", "C"), ncol=1, align="hv", axis = "l")
cowplot::plot_grid(MAE_heatmap_omicron, col2, ncol=2, labels=c("A", ""), axis = "l", rel_widths = c(2.4,1))
